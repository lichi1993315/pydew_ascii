...
class ASCIIRenderer:
	...
	def _is_emoji(self, char):
		...
		code_point = ord(char[0]) if len(char) == 1 else ord(char[0])
	...
	def render_flower_animation(self, surface, pos, frame, variant=0):
		"""
		渲染花朵动画效果
		"""
		# 不同变体的花朵动画字符
		...
		flower_chars = flower_variants[variant % len(flower_variants)]
		char = flower_chars[frame % len(flower_chars)]
		
		# 花朵颜色也可以根据动画帧轻微变化
		base_color = self.color_map['flower']
		r, g, b = base_color
		
		# 根据动画帧调整颜色亮度，创造闪烁效果
		brightness_factor = 0.8 + 0.2 * (frame % 2)
